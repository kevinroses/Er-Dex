...
import { Box, Divider, Flex, Grid, GridItem, Heading, Input, InputGroup, InputRightAddon, SimpleGrid, Text } from '@chakra-ui/react'
          ...
          <Box w="100%" minW="0" overflow="hidden">
            <Box display="flex" gap={2} alignItems="center">
              ...
              <Text
                ...
                ml={2}
              >
                {`${token.address?.slice(0, 6)}...${token.address?.slice(-4)}`} <ExternalLink />
              </Text>
              {isUnknown ? (
                <Box
            ...
            {/* Displaying the token address directly */}
          </Box>
        </Flex>
        <Box flexShrink={0}>
          <Box color={colors.textSecondary} textAlign="right" display="flex" gap="2" alignContent="end">
            {/* First Button */}
            <Text
              width="60px"
              height="24px"
              fontSize="sm"
              padding="2px 8px"
              border="1px solid var(--System-Success-Light)"
              color="var(--System-Success-Light)"
