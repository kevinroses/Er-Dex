    ...
    createClmmPool: async ({ token1, token2, config, price, startTime, execute, forerunCreate, getObserveState }) => {
      ...
      try {
        ...
        const buildData = await raydium.clmm.createPool({
          programId: isDevnet ? DEVNET_PROGRAM_ID.CLMM : programIdConfig.CLMM_PROGRAM_ID,
    ...
    fetchAmmConfigsAct: async () => {
      ...
      try {
        ...
        if (raydium.cluster === 'devnet') {
          res[6].id = 'ED8Ef4V8YLDBHTtuRkt6g59j3kKUnMJkHV2EG9rcwa9R'
        }
