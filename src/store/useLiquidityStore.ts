...
interface LiquidityStore {
  ...
  createPoolAct: (
    ...
    // ) => Promise<string>
  ) => Promise<{ transaction: Transaction | VersionedTransaction }>
  migrateToClmmAct: (
    params: {
      poolInfo: ApiV3PoolInfoStandardItem
  ...
  (set, get) => ({
    ...
    createPoolAct: async ({ pool, baseAmount, quoteAmount, startTime, onSent, onError, onFinally, onConfirmed }) => {
      ...
      const { execute, extInfo, transaction } = await raydium.cpmm.createPool({
        programId: isDevnet ? DEV_CREATE_CPMM_POOL_PROGRAM : programIdConfig.CREATE_CPMM_POOL_PROGRAM,
        poolFeeAccount: isDevnet ? DEV_CREATE_CPMM_POOL_FEE_ACC : programIdConfig.CREATE_CPMM_POOL_FEE_ACC,
